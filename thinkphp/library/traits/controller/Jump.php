...
trait Jump
{
    ...
    protected function success($msg = '', $url = null, $data = '', $wait = 1, array $header = [])
    {
        ...
        if(!empty(session('method_name'))) {
            if(false !== stripos($url, session('method_name') . '/page')) {
                $temp = explode('/', ltrim($url, '/'));
                $page = rtrim(explode('?', $temp[count($temp) - 1])[0], '.html');
                unset($temp[count($temp) - 1]);
                unset($temp[count($temp) - 1]);
                $url = '/' . implode('/', $temp) . '.html';
                if($page > 1) {
                    $page -= 1;
                    $url = '/' . implode('/', $temp) . '.html?page=' . $page;
                }
            }
        }
        $result = [
